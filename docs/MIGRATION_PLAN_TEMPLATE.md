...
**満たさない項目があれば、その対応が移行作業の前に入る。** 引き上げられない項目が
あれば、そこで計画そのものを見直す。

### 2.1 バージョン要件

以下は **Flutter 3.47.1 における値**。要求値はFlutter SDKの更新で変わるため、
着手時に手元のSDKで確認する（2.2節）。

| 項目 | これ未満で**失敗** | これ未満で**警告** | 確認 | 現状値 | 対応 |
|---|---|---|---|---|---|
| Gradle | **8.14.0** | 9.1.0 | `./gradlew -version` | | |
| Android Gradle Plugin | **8.11.1** | 9.0.1 | ルートの `build.gradle` の `classpath` | | |
| Kotlin Gradle Plugin | **2.2.20** | 2.3.20 | 同上。**ホストがKotlinを使う場合のみ** | | |
| Gradleを動かすJDK | **17** | 17 | `./gradlew -version` の `Launcher JVM` | | |
| Android minSdk | **23** | 24 | `app/build.gradle` | | |
| Flutter SDK | — | — | `flutter --version`。iOSでSPMを使うなら**3.44以上** | | |
| Xcode | — | — | `xcodebuild -version`。**15.0以上** | | |
| iOS Deployment Target | — | — | 生成パッケージの宣言値（**15.0**）以上 | | |

**古いアプリで最初に効くのは `minSdk` と iOS の Deployment Target。** どちらも
引き上げると対応OSを切ることになるため、他の項目より先に確認する。

**「警告」の欄は次の引き上げ予告。** 現時点で満たしていなくてもビルドは通るが、
Flutter SDKを上げた時点で失敗に変わる。

**ただし警告に従って上げられるとは限らない。** 依存同士の組み合わせで詰むことが
ある（手順0.2節に実例）。**依存ごとに個別に確かめ、上げられるものだけ上げる。**

### 2.2 手元のSDKから要求値を読む

上の表はSDKに定数として書かれている。**着手時にこのファイルを見るのが確実。**
...
| compileSdk | Flutterモジュールおよびプラグインの要求値以上。下回ると `validateCompileSdkVersion` で失敗する | | |
| プラグインの互換性 | `FlutterPlugin` インターフェースに対応していること | | |
| 対応ABI | Androidは x86_64 / armeabi-v7a / arm64-v8a のみ | | |

要件の背景と確認コマンドはガイド0.1・0.2節。
...
**移行前のアプリがOSごとに違う見た目なら、Flutter画面もそれに合わせないと
その画面だけ浮く。**

| 移行前のアプリ | 判断 |
|---|---|
| 各OSの標準に沿っている（UIKit標準 / Material） | 分けないと移行した画面だけ様式が変わる |
| 独自のデザインシステムで統一されている | 分ける必要がない |

**確認**: ネイティブ側のボタンやフォントの指定を見る。iOSが
`UIButtonTypeSystem` や `systemFont`、Androidが Material の `Button` を
使っているなら、標準に沿っている。

| 方法 | 内容 | コスト |
|---|---|---|
| `.adaptive` コンストラクタ | `CircularProgressIndicator.adaptive()` など。**コードは1つのまま**OSごとに描き分かる | ほぼゼロ。対応部品は限られる |
| Cupertinoウィジェット | `CupertinoPageScaffold` `CupertinoButton` などに差し替える | Viewが2つになる。**全面移行では作業量が倍** |
| 独自デザインで統一 | プラットフォーム標準に合わせない | 1つ。移行前と見た目が変わる |

**条件**: `SnackBar` に iOS の相当物が無い。分ける場合は代替を決める
（`CupertinoAlertDialog` か独自のトースト）。

**条件**: `CupertinoApp` を使う場合、`GlobalCupertinoLocalizations` の追加が要る。
`MaterialApp` のまま中身にCupertinoウィジェットを置く形なら不要。

層を分けてあれば**差し替えるのはViewだけ**で、ViewModel・Repository・Serviceは
共通のまま。
...
| Androidへ組み込む | 3節 | ビルドが通り、**APKに実体が入り、アプリが起動する**（3.5節。構成を明記する） | |
| iOSへ組み込む | 4節 | ビルドが通り、**バンドルに実体が入り、アプリが起動する**（4.5節。構成を明記する） | |
...
**条件**: 完了条件の「ビルドが通る」がDebugなのかReleaseなのかを明記する。
**Debugだけで進めると、Release固有の問題が最後まで見つからない。** 難読化
（R8 / ProGuard）、署名、実際のアプリサイズはDebugでは確認できない。
...
| **Releaseビルド** | **難読化を有効にした状態で動くか。**Debugでしか確認していないと、リリース直前に判明する | |
