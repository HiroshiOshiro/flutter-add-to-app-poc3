...
既存アプリの1画面をFlutter化するときに、着手前に埋める。

## この計画書について

`MIGRATION_GUIDE.md` との分担は次のとおり。

| 資料 | 書いてあること | 性格 |
|---|---|---|
| `MIGRATION_GUIDE.md` | **どうやるか**。条件・手順・確認方法 | プロジェクトに依存しない |
| この計画書 | **今回どう決めたか**。選択・役割分担・範囲・完了条件 | 移行する画面ごとに1部 |

手順は書かない。必要な箇所でガイドの節番号を指す。
...
| 基準 | 理由 |
|---|---|
| 既存データの扱いが読み取りだけ | 書き込みがあると所有者の分割が要る（5節） |
...
後から変えると手戻りが大きいもの。**判断軸を書いてから選ぶ。**

| 決めること | 判断軸 | 選択 | 理由 | 参照 |
|---|---|---|---|---|
| Androidの組み込み方式 | Flutterコードの量ではなく、**Flutterを触らない開発者の割合**。全員が触るならsource module、ホスト側にFlutter SDKを要求できないならAAR | | | 3.2節 |
| iOSの組み込み方式 | 既にCocoaPodsを使っているか。新規ならSPM | | | 4.1節 |
| エンジンの持ち方 | 画面を増やしていく前提か。増やすなら `FlutterEngineGroup` | | | 5.1節 |
| Flutterモジュール名 | **1アプリに1モジュールしか組み込めない。**将来Flutter化するすべての画面の置き場になるため、最初にFlutter化する機能名を付けると2画面目で実態と合わなくなる。ホストアプリに紐づく名前にする | | | 2.2節 |
