...
着手前に決めることは `MIGRATION_PLAN_TEMPLATE.md` に埋める。この資料は手順を
書いたもので、**何を選んだか**はそちらに残す。
...
| 節 | 選ぶこと | 選択肢 | 既定 |
|---|---|---|---|
| 3.2 | Androidの組み込み方式 | source module / AAR | source module |
| 4.1 | iOSの組み込み方式 | SPM / CocoaPods / 手動フレームワーク埋め込み | SPM |
| 5.1 | エンジンの持ち方 | 新規 / キャッシュ / `FlutterEngineGroup` | `FlutterEngineGroup` |

既定はいずれも「画面を増やしていく前提」で選んだもの。1〜2画面で止める場合は
それぞれの節の比較表を見て決める。
